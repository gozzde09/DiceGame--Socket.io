...
fetch(urlM)
    ...
    .then(function(data) {
        ...
        return message.map(function(data) {
            ...
            li.innerHTML = `<strong>${data.user}:</strong>  ${data.message} - <i>${new Date(data.date).toLocaleString()} <i>`;
    ...
    fetch(urlR)
      ...
      .then(function (data) {
        ...
        return result.map(function (data) {
          let li = createNode("li");
          li.innerHTML = `<strong>${data.userName}:</strong> Kastnr ${data.kastNummer} : ${data.kastPoang} / Totalt ${data.totalPoang}   <i>(${new Date(data.date).toLocaleString()})<i>`;
          append(ulResults, li);
        });
      })
      .catch(function (error) {
